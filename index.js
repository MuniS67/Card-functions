...
function busket_reload(ids_arr) {
  ...
  for (let item of temp) {
    ...
    let plus = doc.createElement("p");
    let count = doc.createElement("p");
    let minus = doc.createElement("p");
    ...
    ItemCount.append(plus, count, minus);

    // current
    function currentV(current) {
      plus.onclick = () => {
        current++;
        count.innerHTML = current;
        ItemPrice.innerHTML = `<b>${item.price * current}</b>$`;
      };
      minus.onclick = () => {
        current--;
        count.innerHTML = current;
        ItemPrice.innerHTML = `<b>${
          current * item.price - item.price + item.price
        }</b>$`;
        if (current === 0) {
          temp.splice(item);
          mainItemCard.style.display = "none";
        }
      };
    }
